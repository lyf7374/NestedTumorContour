...
class GroupViTEncoder(nn.Module):
    ...
    def forward(self, x, *, return_attn=False, return_group_token=False):
        ...
        Args:
            ...
            return_attn (bool): Whether to return attention maps
            return_group_token (bool): Whether to return the final group tokens

        Returns:
            Depending on the flags:
                - If `return_group_token` and `return_attn` are False:
                    torch.Tensor: Encoder features of shape [B, L, C]
                - If `return_group_token` is True:
                    Tuple[torch.Tensor, torch.Tensor]: (Encoder features, Group tokens)
                - If `return_attn` is True:
                    Tuple[torch.Tensor, list]: (Encoder features, Attention maps)
                - If both are True:
                    Tuple[torch.Tensor, torch.Tensor, list]: (Encoder features, Group tokens, Attention maps)
        """
        x, group_token, attn_dicts = self.forward_features(x, return_attn=return_attn)
        
        if return_attn and return_group_token:
            return x, group_token, attn_dicts
        elif return_attn:
            return x, attn_dicts
        elif return_group_token:
            return x, group_token
        else:
            return x
...
class UNetDecoder(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        # Remove final_upsample from __init__
    ...
    def forward(self, x, hw_shape):
        ...
        # Upsample to match the input dimensions step by step
        x = self.up1(x)  # Upsample by factor of 2
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        
        x = self.up2(x)  # Upsample by factor of 2
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.relu(x)
...
class GroupViTSegmenter(nn.Module):
    def __init__(self, num_classes, img_size=224, **kwargs):
        ...
        self.decoder = UNetDecoder(in_channels=self.encoder.width, num_classes=num_classes)
        
    def forward(self, x):
        x, group_token, _ = self.encoder(x, return_group_token=True, return_attn=True)
        z = torch.cat([x,group_token],axis=1)

        out = self.decoder(z, None)  # hw_shape is computed in decoder
        return out
